...
def add_pixels_given_centers(x_centers, y_centers, x_array, y_array, image, window_radius):
    for x_center, y_center in zip(x_centers, y_centers):
        x_additional, y_additional = get_pixel_in_window(image, x_center,
                                                         y_center, window_radius)
...
def histogram_pixels_v2(warped_thresholded_image, offset=50, steps=6,
    ...
    for step in range(steps):
        left_x_window_centers = []
        left_y_window_centers = []
        right_x_window_centers = []
        right_y_window_centers = []
        ...
        y_window_center = (window_start_y + window_end_y) // 2
        ...
        if len(left_peaks) > 0:
            ...
            left_y_window_centers.append(y_window_center)

        if len(right_peaks) > 0:
            right_peak = max(right_peaks) + half_frame
            right_x_window_centers.append(right_peak)
            right_y_window_centers.append(y_window_center)
        # else:
        #    plt.plot(histogram)
        # Add coordinates to window centers

        # Get pixels in the left window
        if len(left_x_window_centers) > 0:
            left_x, left_y = add_pixels_given_centers(left_x_window_centers, left_y_window_centers,
                                                  left_x, left_y, warped_thresholded_image, window_radius)

        if len(right_x_window_centers) > 0:
            right_x, right_y = add_pixels_given_centers(right_x_window_centers, right_y_window_centers,
...
def histogram_pixels(warped_thresholded_image, offset=50, steps=6,
    ...
    for step in range(steps):
        ...
        # Get pixels in the left window
        for left_x_center, y_center in zip(left_x_window_centers, y_window_centers):
            left_x_additional, left_y_additional = get_pixel_in_window(warped_thresholded_image, left_x_center,
                                                                       y_center, window_radius)
        ...
        for right_x_center, y_center in zip(right_x_window_centers, y_window_centers):
            right_x_additional, right_y_additional = get_pixel_in_window(warped_thresholded_image, right_x_center,
                                                                         y_center, window_radius)
...
def add_figures_to_image2(img, curvature, vehicle_position, min_curvature, left_coeffs=(0,0,0), right_coeffs=(0,0,0)):
    """
...
def histogram_pixels(warped_thresholded_image, offset=50, steps=6,
    ...
    for step in range(steps):
        ...
        # Get pixels in the left window
        for left_x_center, y_center in zip(left_x_window_centers, y_window_centers):
            left_x_additional, left_y_additional = get_pixels(warped_thresholded_image, left_x_center,
                                                                       y_center, window_radius)
            # plt.scatter(left_x_additional, left_y_additional)
            # Add pixels to list
            left_x.append(left_x_additional)
        ...
        # Get pixels in the right window
        for right_x_center, y_center in zip(right_x_window_centers, y_window_centers):
            right_x_additional, right_y_additional = get_pixels(warped_thresholded_image, right_x_center,
                                                                         y_center, window_radius)
            # plt.scatter(right_x_additional, right_y_additional)
            # Add pixels to list
            right_x.append(right_x_additional)
...
#######################################################################
def add_figures_to_image3(img, curvature, vehicle_position, min_curvature, left_coeffs=(0,0,0), right_coeffs=(0,0,0)):
